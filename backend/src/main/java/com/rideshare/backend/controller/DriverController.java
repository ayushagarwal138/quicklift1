...
public class DriverController {
    ...
    public ResponseEntity<?> getOnlineDrivers() {
        List<Driver> onlineDrivers = driverRepository.findByStatus(com.rideshare.backend.model.DriverStatus.ONLINE);
    ...
    public ResponseEntity<?> getDriverSummary() {
        ...
        double earnings = driver.getTrips().stream()
                .filter(trip -> trip.getStatus() == com.rideshare.backend.model.TripStatus.COMPLETED && trip.getFare() != null)
                .mapToDouble(trip -> trip.getFare().doubleValue())
                .sum();
        double avgRating = driver.getTrips().stream()
                .filter(trip -> trip.getStatus() == com.rideshare.backend.model.TripStatus.COMPLETED && trip.getRating() != null)
